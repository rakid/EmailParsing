...
from mcp.types import Resource, Tool, Prompt, TextResourceContents, PromptMessage, TextContent
import asyncio
import json
import sys
import os
...
try:
    from .integrations import integration_registry, ExportFormat, DataExporter, DatabaseInterface, AIAnalysisInterface, PluginInterface
...
async def handle_list_resources() -> list[Resource]:
    ...
    return [
        ...
        Resource(
            ...
            mimeType="application/json"
        ) for email_id in list(storage.email_storage.keys())[:10]  # Limit to recent emails
    ]
...
async def handle_read_resource(uri: str) -> str:
    ...
    if uri == "email://processed":
        ...
        high_urgency_emails = [
            email for email in storage.email_storage.values()
        ...
        for email in storage.email_storage.values():
            if email.analysis and email.analysis.urgency_score >= 40:  # Default threshold
                task_data = {
                    "email_id": email.id,
...
async def handle_list_tools() -> list[Tool]:
    ...
    tools = [
        ...
        ] if INTEGRATIONS_AVAILABLE else []
    )
...
async def handle_call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    if name == "analyze_email":
        ...
        try:
            if email_id and email_id in storage.email_storage:
                ...
                if processed_email.analysis:
                    ...
                    return [TextContent(type="text", text=f"Email {email_id} found but not yet analyzed")]
            else:
                # Analyze provided content
                if not content:
                    return [TextContent(type="text", text="Error: Either email_id or content must be provided")]
        ...
        try:
            ...
            for email in storage.email_storage.values():
                # Apply filters
                if urgency_level and email.analysis and email.analysis.urgency_level.value != urgency_level:
                    continue
                if sentiment and email.analysis and email.analysis.sentiment != sentiment:
                    continue
                
                # Apply text search
                if query:
                    searchable_text = f"{email.email_data.subject} {email.email_data.text_body or ''}"
        ...
        try:
            total_emails = len(storage.email_storage)
            analyzed_emails = sum(1 for email in storage.email_storage.values() if email.analysis)
            ...
            stats_result = {
                ...
                "avg_processing_time": sum(storage.stats.processing_times) / len(storage.stats.processing_times) if storage.stats.processing_times else 0
            }
            ...
            if include_distribution and analyzed_emails > 0:
                ...
                stats_result.update({
                    ...
                    "min_urgency_score": min(urgency_scores) if urgency_scores else 0
                })
        ...
        try:
            ...
            if email_id:
                ...
                if email_id in storage.email_storage:
                    ...
                    if email.analysis and email.analysis.urgency_score >= urgency_threshold:
                        task_data = {
                            "email_id": email_id,
                            "from": email.email_data.from_email,
                            "subject": email.email_data.subject,
    ...
    # Integration tools (available only if integrations module is loaded)
        ...
        try:
            ...
            exported_file = DataExporter.export_emails(emails_to_export, export_format_enum, filename)
        ...
        try:
            ...
            return [TextContent(type="text", text=f"Integration listing error: {str(e)}")]
        ...
        try:
            ...
            processed_email = await integration_registry.plugin_manager.process_email_through_plugins(original_email)
            ...
            result = {
                ...
                "original_tags": original_email.analysis.tags if original_email.analysis else [],
                "updated_tags": processed_email.analysis.tags if processed_email.analysis else [],
...
async def handle_list_prompts() -> list[Prompt]:
    ...
    return [
        Prompt(
            ...
            arguments=[
                {"name": "email_content", "description": "The email content to analyze"},
                {"name": "analysis_type", "description": "Type of analysis (urgency, sentiment, tasks)"}
            ]
        )
    ]
