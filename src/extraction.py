...
class EmailExtractor:
    ...
    def _compile_patterns(self) -> Dict[str, Any]:
        ...
        for level, patterns in self.URGENCY_PATTERNS.items():
            compiled['urgency'][level] = [re.compile(pattern, re.IGNORECASE) for pattern in patterns]
        
        # Compile other patterns
        compiled['time'] = [re.compile(pattern, re.IGNORECASE) for pattern in self.TIME_PATTERNS]
        compiled['actions'] = [re.compile(pattern, re.IGNORECASE) for pattern in self.ACTION_PATTERNS]
        
        # Compile contact patterns
        compiled['contact'] = {}
        for contact_type, patterns in self.CONTACT_PATTERNS.items():
            compiled['contact'][contact_type] = [re.compile(pattern, re.IGNORECASE) for pattern in patterns]
        
        # Compile sentiment patterns
        compiled['sentiment'] = {}
        for sentiment, patterns in self.SENTIMENT_PATTERNS.items():
            compiled['sentiment'][sentiment] = [re.compile(pattern, re.IGNORECASE) for pattern in patterns]
                ...
                if matches:
                    ...
                    for match in matches:
                        if isinstance(match, tuple):
                            ...
                            match_str = next((group for group in match if group), str(match[0]))
                        else:
                            match_str = str(match)
        ...
        for pattern in self.compiled_patterns['time']:
            ...
            for match in matches:
                if isinstance(match, tuple):
                    ...
                    match_str = next((group for group in match if group), " ".join(match))
                else:
                    match_str = str(match)
    ...
    def calculate_urgency_score(self, urgency_indicators: Dict[str, List[str]]) -> Tuple[int, str]:
