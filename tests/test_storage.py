...
class TestEmailStorage:
    ...
    def test_retrieve_email(self, sample_email_data):
        ...
        email_data = EmailData(**sample_email_data)
        processed_email = ProcessedEmail(id="retrieve-test", email_data=email_data)
    ...
    def test_update_email(self, sample_email_data, sample_analysis_data):
        ...
        processed_email = ProcessedEmail(
            id="update-test", email_data=email_data, status=EmailStatus.RECEIVED
        )
    ...
    def test_delete_email(self, sample_email_data):
        ...
        email_data = EmailData(**sample_email_data)
        processed_email = ProcessedEmail(id="delete-test", email_data=email_data)
    ...
    def test_clear_storage(self, sample_email_data):
        ...
        for i in range(5):
            email_data = EmailData(
                **{**sample_email_data, "message_id": f"clear-test-{i}"}
            )
            processed_email = ProcessedEmail(
                id=f"clear-test-{i}", email_data=email_data
            )
    ...
    def test_storage_iteration(self, sample_email_data):
        ...
        for i in range(3):
            ...
            email_data = EmailData(**{**sample_email_data, "message_id": f"iter-{i}"})
...
class TestEmailStats:
    ...
    def test_stats_processing_times(self):
        ...
        avg_time = sum(storage.stats.processing_times) / len(
...
class TestStorageIntegration:
    ...
    def test_storage_with_stats_update(self, sample_email_data, sample_analysis_data):
        ...
        storage.stats.urgency_distribution = {UrgencyLevel.HIGH: 1, UrgencyLevel.LOW: 1}
    ...
    def test_concurrent_access_simulation(self, sample_email_data):
        ...
        def store_email(email_id, delay=0):
            ...
            email_data = EmailData(
                **{**sample_email_data, "message_id": f"concurrent-{email_id}"}
            )
            processed_email = ProcessedEmail(
                id=f"concurrent-{email_id}", email_data=email_data
            )
    ...
    def test_storage_memory_usage(self, sample_email_data):
        ...
        for i in range(num_emails):
            ...
            processed_email = ProcessedEmail(
                id=f"memory-test-{i}", email_data=email_data
            )
    ...
    def test_storage_persistence_simulation(self, sample_email_data):
        ...
        for i in range(3):
            email_data = EmailData(
                **{**sample_email_data, "message_id": f"persist-{i}"}
            )
            processed_email = ProcessedEmail(id=f"persist-{i}", email_data=email_data)
