...
class TestEndToEndIntegration:
    ...
    async def test_complete_postmark_to_mcp_workflow(self, mock_extractor, mock_config):
        ...
        mock_config.postmark_webhook_secret = (
            None  # Disable signature verification for tests
        )
...
""",
        ...
        assert (
            stored_email.analysis.urgency_level == UrgencyLevel.CRITICAL
            or stored_email.analysis.urgency_level == UrgencyLevel.HIGH
        )
        ...
        email_resource_result = await server.handle_read_resource(
    ...
    async def test_mcp_tools_integration(self, mock_config, mock_extractor):
        ...
        tasks_result = await server.handle_call_tool(
            # The urgent email
            "extract_tasks",
            {"email_id": processing_ids[0]},
        )
        assert len(tasks_result) > 0
        tasks_content = json.loads(tasks_result[0].text)
        # Should extract tasks from urgent email
        assert len(tasks_content["tasks"]) > 0
    ...
    def test_error_handling_and_recovery(self, mock_config, mock_extractor):
        ...
        assert response.status_code in [
            422,
            500,
        ]  # Validation error (could be 500 due to internal handling)
...
- Meeting scheduled for today at 3 PM
        ...
        assert (
            processing_time < 2.0
        ), f"Processing took {processing_time:.3f}s, should be under 2s"
    ...
    def test_memory_usage_stability(self, mock_config, mock_extractor):
        ...
        assert (
            memory_growth < 100
        ), f"Memory grew by {memory_growth:.2f}MB, should be under 100MB"
...
class TestMCPProtocolCompliance:
    ...
    async def test_mcp_server_initialization(self):
        ...
        assert (
            "MCP server for unified email entry, parsing, and analysis"
            in server.server.instructions
        )
