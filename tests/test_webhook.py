...
class TestEmailDataExtraction:
    ...
    def test_extract_email_data_basic(self, sample_postmark_payload):
        ...
        assert email_data.subject == "URGENT: Project deadline tomorrow - need your input ASAP"
...
class TestWebhookProcessing:
    ...
    def test_webhook_processing_success(self, mock_logger, mock_extractor, mock_config, sample_postmark_payload):
    ...
    @patch('src.webhook.config')
    def test_webhook_signature_verification_required(self, mock_config, sample_postmark_payload):
    ...
    @patch('src.webhook.config')
    @patch('src.webhook.email_extractor')
    def test_webhook_stats_update(self, mock_extractor, mock_config, sample_postmark_payload):
...
class TestWebhookIntegration:
    ...
    @patch('src.webhook.email_extractor')
    @patch('src.webhook.logger')
    def test_complete_email_processing_flow(self, mock_logger, mock_extractor, mock_config, sample_postmark_payload):
        ...
        assert stored_email.email_data.subject == "URGENT: Project deadline tomorrow - need your input ASAP"
    ...
    def test_concurrent_webhook_processing(self, sample_postmark_payload):
        ...
        with patch('src.webhook.config') as mock_config, \
            ...
            def process_webhook(message_id):
                payload = {**sample_postmark_payload, "MessageID": f"concurrent-{message_id}"}
                response = client.post("/webhook", json=payload)
