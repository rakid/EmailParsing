...
class TestAttachmentData:
    ...
    def test_attachment_validation_missing_fields(self):
        ...
        with pytest.raises(ValidationError):
            AttachmentData(name="test.pdf", content_type="application/pdf")  # Missing size
...
class TestEmailData:
    ...
    def test_email_data_creation(self, sample_email_data):
        ...
        assert email.subject == "URGENT: Project deadline tomorrow - need your input ASAP"
    ...
    def test_email_data_with_optional_fields(self):
        ...
        email_data = {
            ...
            "attachments": [
                AttachmentData(name="test.pdf", content_type="application/pdf", size=1024)
            ],
...
class TestEmailAnalysis:
    ...
    def test_analysis_score_validation(self):
        ...
        analysis = EmailAnalysis(
            urgency_score=0, urgency_level=UrgencyLevel.LOW,
            sentiment="neutral", confidence=0.5
        )
        assert analysis.urgency_score == 0
        
        analysis = EmailAnalysis(
            urgency_score=100, urgency_level=UrgencyLevel.CRITICAL,
        ...
        with pytest.raises(ValidationError):
            EmailAnalysis(
                urgency_score=-1, urgency_level=UrgencyLevel.LOW,
                sentiment="neutral", confidence=0.5
            )
        
        with pytest.raises(ValidationError):
            EmailAnalysis(
                urgency_score=101, urgency_level=UrgencyLevel.HIGH,
    ...
    def test_confidence_validation(self):
        ...
        analysis = EmailAnalysis(
            urgency_score=50, urgency_level=UrgencyLevel.MEDIUM,
        ...
        with pytest.raises(ValidationError):
            EmailAnalysis(
                urgency_score=50, urgency_level=UrgencyLevel.MEDIUM,
                sentiment="neutral", confidence=-0.1
            )
        
        with pytest.raises(ValidationError):
            EmailAnalysis(
                urgency_score=50, urgency_level=UrgencyLevel.MEDIUM,
                sentiment="neutral", confidence=1.1
            )
...
class TestPostmarkWebhookPayload:
    ...
    def test_postmark_payload_creation(self, sample_postmark_payload):
        ...
        assert payload.Subject == "URGENT: Project deadline tomorrow - need your input ASAP"
    ...
    def test_postmark_payload_with_optional_fields(self):
        ...
        payload_data = {
            ...
            "Attachments": [
                {"Name": "test.pdf", "ContentType": "application/pdf", "ContentLength": 1024}
            ],
            "Tag": "test-tag",
...
class TestModelIntegration:
    ...
    def test_complete_email_processing_flow(self, sample_email_data, sample_analysis_data):
    ...
    def test_model_serialization_deserialization(self, sample_email_data, sample_analysis_data):
    ...
    def test_postmark_to_email_data_conversion(self, sample_postmark_payload):
        ...
        email_data = EmailData(
            ...
            received_at=datetime.fromisoformat(postmark_payload.Date.replace('Z', '+00:00')),
            headers={header["Name"]: header["Value"] for header in postmark_payload.Headers}
        )
