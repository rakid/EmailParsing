...
class TestResourceHandling:
    ...
    async def test_read_recent_emails_resource(self, sample_email_data):
        ...
        for i in range(15):  # More than the default limit of 10
            ...
            processed_email = ProcessedEmail(id=f"recent-{i}", email_data=email_data)
    ...
    @pytest.mark.asyncio
    async def test_read_analytics_resource(
        self, sample_email_data, sample_analysis_data
    ):
    ...
    async def test_read_tasks_resource(self, sample_email_data, sample_analysis_data):
        ...
        # Only 50 and 70 should be included (>= 40)
...
class TestToolHandling:
    ...
    async def test_search_emails_tool(self, sample_email_data, sample_analysis_data):
        ...
        email_configs = [
            (
                ...
                UrgencyLevel.HIGH,
                "positive",
                "We need to discuss the project in our meeting.",
            ),
            (
                "search-2",
                "Invoice payment due",
                UrgencyLevel.MEDIUM,
                "neutral",
                "Please process the invoice payment by end of week.",
            ),
            (
                "search-3",
                ...
                UrgencyLevel.LOW,
                "positive",
                "The team meeting went well and we made good progress.",
            ),
        ]
        ...
        result = await server.handle_call_tool(
            "search_emails", {"sentiment": "positive"}
        )
        response_data = json.loads(result[0].text)
        assert response_data["total_found"] == 2
    ...
    async def test_get_email_stats_tool(self, sample_email_data, sample_analysis_data):
        ...
        for i in range(3):
            analysis = EmailAnalysis(**sample_analysis_data)
            email_data = EmailData(**{**sample_email_data, "message_id": f"stats-{i}"})
            processed_email = ProcessedEmail(
                id=f"stats-{i}", email_data=email_data, analysis=analysis
            )
    ...
    async def test_extract_tasks_tool_all_emails(
        ...
        for i, score in enumerate(urgency_scores):
            ...
            processed_email = ProcessedEmail(
                id=f"extract-{i}", email_data=email_data, analysis=analysis
            )
    ...
    async def test_extract_tasks_tool_specific_email(
        ...
        email_data = EmailData(**sample_email_data)
        processed_email = ProcessedEmail(
            id="specific-task", email_data=email_data, analysis=analysis
        )
...
class TestErrorHandling:
    ...
    async def test_tool_error_handling(self):
        ...
        result = await server.handle_call_tool(
            "extract_tasks", {"email_id": "nonexistent"}
        )
    ...
    async def test_analysis_error_handling(self):
        ...
        with patch("src.server.email_extractor") as mock_extractor:
            ...
            result = await server.handle_call_tool(
                "analyze_email", {"content": "Test content"}
            )
