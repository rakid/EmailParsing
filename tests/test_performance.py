...
class TestConcurrentProcessing:
    ...
    def test_concurrent_webhook_processing(self):
        ...
        def process_emails(thread_id: int) -> List[Dict[str, Any]]:
            ...
            for i in range(emails_per_thread):
                ...
                extracted_metadata = extractor.extract_from_email(email_data)
                urgency_score, _ = extractor.calculate_urgency_score(
                    extracted_metadata.urgency_indicators
                )
        ...
        avg_processing_time = sum(r["processing_time"] for r in all_results) / len(
            all_results
        )

        print("Concurrent processing stats:")
        ...
        ), f"Individual email processing time {
            max_processing_time:.4f}s exceeds 2s limit"
        assert (
            avg_processing_time < 1.0
        ), f"Average processing time {avg_processing_time:.4f}s is too high"
    ...
    def test_concurrent_mcp_tool_calls(self):
        ...
        print("Concurrent MCP tool calls:")
        ...
        assert (
            successful_calls >= 6
        ), f"Too many failed tool calls: {successful_calls}/{len(results)}"
        assert (
            total_time < 10.0
        ), f"Concurrent tool calls took too long: {total_time:.4f}s"
    ...
    def test_memory_usage_stability(self):
        ...
        for i in range(num_emails):
            email_data = EmailData(
                ...
                text_body=f"This is test email {i} for memory usage testing. "
                * 10,  # Larger content
            ...
            # Process email
            extractor.extract_from_email(email_data)
        ...
        assert (
            memory_increase / num_emails < 0.5
        ), f"Memory per email {memory_increase / num_emails:.3f} MB is too high"
    ...
    def test_storage_cleanup_effectiveness(self):
        ...
        for i in range(50):
            email_data = EmailData(
                ...
                text_body="Large content for cleanup testing. " * 50,  # Large content
        ...
        print("Storage cleanup analysis:")
        ...
        print("  Storage items cleared: 50 emails")
...
class TestScalabilityLimits:
    ...
    def test_storage_capacity_limits(self):
        ...
        assert (
            ...
            avg_time_per_email:.4f}s is too high for capacity test"
        ...
        print(
            f"  Sample retrieval time: {
                retrieval_time:.4f}s for {retrieved_emails} emails"
        )
